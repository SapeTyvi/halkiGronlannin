 ...
 */
function ewp_customize_register( $wp_customize ) {
	...
	$wp_customize->add_control(
		new WP_Customize_Color_Control(
			...
			'theme_bg_color', 
				array(
					'label'		=> __( 'Header ja footerin taustaväri', 'ewp'),
					'section'	=> 'colors',
	...
	$wp_customize->add_section( 'theme_options', 
		array(
			'title'			=> __( 'Julkaisu asetukset', 'ewp' ),
			'priority'		=> 95,
			'capability'	=> 'edit_theme_options',
			'description'	=> __( 'Valitse näytetäänkö julkaisusta index ja archive sivuilla ote vai koko sisältö.', 'ewp' )
		)
	);
	...
	$wp_customize->add_setting(	'length_setting',
		array(
			'default'			=> 'excerpt',
			'type'				=> 'theme_mod',
			'sanitize_callback' => 'ewp_sanitize_length', // Sanitization function appears further down
			'transport'			=> 'postMessage'
		)
	);

	// Add the controls
	$wp_customize->add_control(	'ewp_length_control',
		array(
			'type'		=> 'radio',
			'label'		=> __( 'Index/archive näyttää', 'ewp' ),
			'section'	=> 'theme_options',
			'choices'	=> array(
				'excerpt'		=> __( 'Näytä ote (default)', 'ewp' ),
				'full-content'	=> __( 'Näytä koko julkaisu', 'ewp' )
			),
...
add_action( 'customize_register', 'ewp_customize_register' );

/**
 * Binds JS handlers to make Theme Customizer preview reload changes asynchronously.
 */
function ewp_customize_preview_js() {
	wp_enqueue_script( 'ewp_customizer', get_template_directory_uri() . '/js/customizer.js', array( 'customize-preview' ), '20151215', true );
}
add_action( 'customize_preview_init', 'ewp_customize_preview_js' );
...
function ewp_sanitize_length( $value ) {
...
if ( ! function_exists( 'ewp_header_style' ) ) :
/**
 * Styles the header image and text displayed on the blog.
 *
 * @see ewp_custom_header_setup().
 */
function ewp_header_style() {
